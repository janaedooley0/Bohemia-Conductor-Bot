...
class JotFormHelper:
    ...
    def _parse_payment_items(self, answer, pretty_format):
        ...
        if pretty_format:
            cleaned_pretty = re.sub(r'<[^>]+>', '', str(pretty_format))
            lines = [line.strip() for line in cleaned_pretty.splitlines() if line.strip()]
            for line in lines:
                match = re.match(
...
def format_order_display(order_data):
    ...
    """
    form_title = html.escape(order_data.get('form_title', 'Group Buy'))
    telegram_username = html.escape(order_data.get('telegram_username', 'N/A'))
    customer_name = html.escape(order_data.get('customer_name', ''))
    invoice_id = html.escape(order_data.get('invoice_id', 'N/A'))
    ...
    if products:
        ...
        for i, product in enumerate(products, 1):
            name = html.escape(product.get('name', 'Unknown Item'))
